...
var gizmo: PackedScene = preload("res://scenes/ingame/gizmo.tscn")
var gizmo_instance: Node3D = null
...
func _process(_delta: float) -> void:
	...
	if not Input.is_action_pressed("lmb"):
		...
		return
	if selected:
		gizmo_instance.global_position = selected.global_position
		gizmo_instance.global_rotation_degrees = Vector3.ZERO
	...
	var move_dir: String = "+x"
	match gizmove_dir:
		Vector3(1, 0, 0): move_dir = "+x"
		Vector3(0, 1, 0): move_dir = "+y"
		Vector3(0, 0, 1): move_dir = "+z"
		Vector3(-1, 0, 0): move_dir = "-x"
		Vector3(0, -1, 0): move_dir = "-y"
		Vector3(0, 0, -1): move_dir = "-z"
	...
	match gizmove_type: # Least confusing equations:
		"move": selected.position = snapped(gizmove_origin - ((mouse_y - gizmove_start) * 0.1 * gizmove_dir), increment * gizmove_dir)
		"size":
			selected.size = snapped(gizmove_origin - ((mouse_y - gizmove_start) * 0.05 * abs(gizmove_dir) ), increment) # No clue how I made this lmao
			match move_dir:
				"+x", "-x": selected.size.x = clamp(selected.size.x, increment.x, 999999)
				"+y", "-y": selected.size.y = clamp(selected.size.y, increment.y, 999999)
				"+z", "-z": selected.size.z = clamp(selected.size.z, increment.z, 999999)
			if Input.is_action_pressed("even"): selected.position = gizmove_pos
			else: selected.position = snapped(gizmove_pos + (gizmove_dir * (mouse_y - gizmove_start) * -0.025), increment * 0.5 * gizmove_dir)
		"scale":
			if selected.is_in_group("orb"): return
			selected.scale = snapped(gizmove_origin - ((mouse_y - gizmove_start) * 0.05 * abs(gizmove_dir)), increment)
			match gizmove_dir:
				Vector3(1, 0, 0), Vector3(-1, 0, 0): selected.scale.x = clamp(selected.scale.x, increment.x, 999999)
				Vector3(0, 1, 0), Vector3(0, -1, 0): selected.scale.y = clamp(selected.scale.y, increment.y, 999999)
				Vector3(0, 0, 1), Vector3(0, 0, -1): selected.scale.z = clamp(selected.scale.z, increment.z, 999999)
			if Input.is_action_pressed("even"): selected.position = gizmove_pos
			else: selected.position = gizmove_pos + snapped((gizmove_dir * (mouse_y - gizmove_start) * -0.025), increment * 0.5 * gizmove_dir)
		"rotate": selected.rotation_degrees = snapped(gizmove_origin - ((mouse_y - gizmove_start) * gizmove_dir), Vector3(rotation_increment, rotation_increment, rotation_increment) * gizmove_dir)
...
func _on_lock_cursor_pressed() -> void:
	...
	if selected:
		...
		for child in selected.get_children(): if child.is_in_group("gizmo"): child.queue_free()
	selected = null
...
func selection(object: Node) -> void:
	...
	if selected: for child in selected.get_children(): if child.is_in_group("gizmo"): child.queue_free()
	selected = null

	if object:
		selected = object
		gizmo_instance = gizmo.instantiate()
		selected.add_child(gizmo_instance)
		
		gizmo_instance.global_rotation_degrees = Vector3.ZERO
		gizmo_instance.global_position = selected.global_position
		gizmo_instance.owner = null
...
func handle_duplicate() -> void:
	...
	gizmo_instance = gizmo.instantiate()
	duplicated.add_child(gizmo_instance)
