...
func _on_scale1_text_submitted(new_text: String) -> void:
	...
	if root.selected.is_in_group("orb"):
		root.selected.scale.y = float(new_text)
		new_text_value(_scale2, root.selected.scale.y)
		
		root.selected.scale.z = float(new_text)
...
func _on_scale2_text_submitted(new_text: String) -> void:
	...
	if root.selected.is_in_group("orb"):
		root.selected.scale.x = float(new_text)
		new_text_value(_scale1, root.selected.scale.x)
...
func _on_orb_type_dropdown_item_selected(index: int) -> void:
	match index:
		0: root.selected.type = "none"
		2: root.selected.type = "dash"
		3: root.selected.type = "jump"
		5: root.selected.type = "stat_speed"
		6: root.selected.type = "stat_jump"
		8: root.selected.type = "fuel"
		9: root.selected.type = "end"
		
	$scroll/vbox/orb_type/dropdown.selected = index
	root.selected.load_texture()
	
func _on_orb_value_text_submitted(new_text: String) -> void:
	if new_text.is_valid_float():
		root.selected.value = float(new_text)
		$scroll/vbox/orb_value/value.text = str(float(new_text))
...
func _on_material_dropdown_item_selected(index: int) -> void:
	...
	# This is also gonna be long as hell, and have a huge blast radius, but IDK how else to do it. Probably possible to find the text of the index.
	match index:
